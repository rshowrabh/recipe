    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <a href="{{ url('/admin/meal-dish-type/create') }}" class="btn btn-primary float-right">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <table id="example" class="table table-bordered table-striped">
                    ...
                    <tbody>
                    @foreach($mealdishtype as $item)
                        <tr>
                            <td>{{ $item->name }}</td>
                            <td>{!! $item->description !!}</td>
                            <td>
                                <a href="{{ url('/admin/meal-dish-type/' . $item->id) }}" title="View MealDishType">
                                    <button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i>
                                        View
                                    </button>
                                </a>
                                <a href="{{ url('/admin/meal-dish-type/' . $item->id . '/edit') }}"
                                   title="Edit MealDishType">
                                    <button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o"
                                                                              aria-hidden="true"></i> Edit
                                    </button>
                                </a>

                                <form method="POST" action="{{ url('/admin/meal-dish-type' . '/' . $item->id) }}"
                                      accept-charset="UTF-8" style="display:inline">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
                                    <button type="submit" class="btn btn-danger btn-sm" title="Delete MealDishType"
