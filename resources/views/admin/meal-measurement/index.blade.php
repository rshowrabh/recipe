    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <a href="{{ url('/admin/meal-measurement/create') }}" class="btn btn-primary float-right">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <table id="example" class="table table-bordered table-striped">
                    ...
                    <tbody>
                    @foreach($mealmeasurement as $item)
                        <tr>
                            <td>{{ $item->unit }}</td>
                            <td>
                                <a href="{{ url('/admin/meal-measurement/' . $item->id) }}"
                                   title="View MealMeasurement">
                                    <button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i>
                                        View
                                    </button>
                                </a>
                                @canany(['isAdmin','isEditor'])
                                <a href="{{ url('/admin/meal-measurement/' . $item->id . '/edit') }}"
                                   title="Edit MealMeasurement">
                                ...
                                <form method="POST" action="{{ url('/admin/meal-measurement' . '/' . $item->id) }}"
                                      accept-charset="UTF-8" style="display:inline">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
                                    <button type="submit" class="btn btn-danger btn-sm" title="Delete MealMeasurement"
