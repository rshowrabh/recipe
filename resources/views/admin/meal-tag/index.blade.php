    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <a href="{{ url('/admin/meal-tag/create') }}" class="btn btn-primary float-right">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <table id="example" class="table table-bordered table-striped">
                    ...
                    <tbody>
                    @foreach($mealtag as $item)
                        <tr>
                            <td>{{ $item->name }}</td>
                            <td>{!! $item->description !!}</td>
                            <td>{{ $item->approved }}</td>
                            <td>{{ $item->color_code }}</td>
                            <td>{{ $item->meal_tag_categories_id }}</td>
                            <td>
                                <a href="{{ url('/admin/meal-tag/' . $item->id) }}" title="View MealTag">
                                    <button class="btn btn-info btn-sm"><i class="fa fa-eye" aria-hidden="true"></i>
                                        View
                                    </button>
                                </a>
                                @canany(['isAdmin','isEditor'])
                                <a href="{{ url('/admin/meal-tag/' . $item->id . '/edit') }}" title="Edit MealTag">
                                ...
                                <form method="POST" action="{{ url('/admin/meal-tag' . '/' . $item->id) }}"
                                      accept-charset="UTF-8" style="display:inline">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
                                    <button type="submit" class="btn btn-danger btn-sm" title="Delete MealTag"
